...
brick_tower_inner_part();

module brick_tower_inner_part()
{
  ...
  mask_height = BRICK_CALCULATE_PHYSICAL_HEIGHT_MASK(6 + 1);
  difference()
  {

    // cyl(d = BRICK_CALCULATE_PHYSICAL_LENGTH(8), h = BRICK_CALCULATE_PHYSICAL_HEIGHT(6), anchor = BOT);
    brick_circle(outer_size = 8, inner_size = 6, height = 6, texture = texture, tex_size = tex_size, tex_scale = 0.5);
    ...
    // #down(0.001) back(length / 2) right(length / 2) cuboid([ mask_length2, mask_length1, mask_height ], anchor =
    // BOT);
  }
}

module brick_tower_outer_part()
{
...
// HER
// size = 8;
// physical_size = BRICK_CALCULATE_PHYSICAL_LENGTH(8);
// outer_shape = random_polygon(5, physical_size / 2, 10009);
// echo(outer_shape);
// // region(outer_shape);
// brick_from_shape(size, size, 1, outer_shape);
...
//       cyl(d = 30, h = BRICK_CALCULATE_PHYSICAL_HEIGHT(1) - BRICK_CALCULATE_PHYSICAL_ROOF_THICKNESS(1), anchor =
//       BOT);
